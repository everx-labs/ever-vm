...
%\usepackage{chngcntr}
%\CompileMatrices
\usepackage[bookmarksnumbered,pdfauthor={EverX},pdftitle={Telegram Open Network Virtual Machine}]{hyperref}
\usepackage{fancyhdr}
\usepackage{caption}
%
...
\nxsubpoint\emb{Diff operations (Only available in GOSH network)}
\begin{itemize}
\item {\tt C714} --- {\tt DIFF} ($c$ $c$ -- $c$), calculates diff between first and second parameters and returns it as cell.
\item {\tt C715} --- {\tt DIFF\_PATCH} ($c$ $c$ -- $c$), takes the first parameter as document, the second parameter as patch, received on DIFF command, applies patch and saves result document to cell.
\item {\tt C716} --- {\tt ZIP} ($c$ -- $c$), zip string to cell
\item {\tt C717} --- {\tt UNZIP} ($c$ -- $c$), unzip cell to string
\item {\tt C718} --- {\tt DIFF\_ZIP} ($c$ $c$ -- $c$), like DIFF but unzip parameters before operation and zip to cell after operation.
\item {\tt C719} --- {\tt DIFF\_PATCH\_ZIP} ($c$ $c$ -- $c$), like DIFF\_PATCH but unzip parameters before operation and zip to cell after operation.
\item {\tt C720} --- {\tt DIFF\_PATCHQ} ($c$ $c$ -- $c$), quiet version of DIFF\_PATCH
\item {\tt C721} --- {\tt DIFF\_PATCH\_ZIPQ} ($c$ $c$ -- $c$), quiet version of DIFF\_PATCH\_ZIP
\item {\tt C722} --- {\tt DIFF\_PATCH\_BINARY} ($c$ $c$ -- $c$), like DIFF\_PATCH but takes and returns binary arrays instead of string.
\item {\tt C723} --- {\tt DIFF\_PATCH\_BINARY\_ZIP} ($c$ $c$ -- $c$), like DIFF\_PATCH\_ZIP but takes and returns binary arrays instead of string.
\item {\tt C724} --- {\tt DIFF\_PATCH\_BINARYQ} ($c$ $c$ -- $c$), like DIFF\_PATCHQ but takes and returns binary arrays instead of string.
\item {\tt C725} --- {\tt DIFF\_PATCH\_BINARY\_ZIPQ} ($c$ $c$ -- $c$), like DIFF\_PATCH\_ZIPQ but takes and returns binary arrays instead of string.
\end{itemize}
...
\item {\tt D766} --- {\tt LDCONT} ($s$ -- $cont$ $s'$), loads a continuation $cont$ from $s$.
\end{itemize}
...
\item {\tt F93000} --- {\tt BLS\_VERIFY} ($pk$ $msg$ $sgn$ -- $?$) Checks BLS signature, return true on success, false otherwise.
\item {\tt F93001} --- {\tt BLS\_AGGREGATE} ($sig_1$ \ldots $sig_n$ $n$ -- $sig$) Aggregates signatures. $n>0$. Throw exception if $n=0$ or if some $sig_i$ is not a valid signature.
\item {\tt F93002} --- {\tt BLS\_FASTAGGREGATEVERIFY} ($pk_1$ \ldots $pk_n$ $n$ $msg$ $sig$ -- $?$) Checks aggregated BLS signature for keys $pk_1\dotspk_n$ and message $msg$. Return true on success, false otherwise. Return false if $n=0$
\item {\tt F93003} --- {\tt BLS\_AGGREGATEVERIFY} ($pk_1$ $msg_1$ \ldots $pk_n$ $msg_n$ $n$ $sgn$ -- $?$) Checks aggregated BLS signature for key-message pairs $pk_1 msg_1\dotspk_n msg_n$. Return true on success, false otherwise. Return false if $n=0$
\item {\tt F93010} --- {\tt BLS\_G1\_ADD} ( $x$ $y$ -- $x$+$y$) Addition on G1.
\item {\tt F93011} --- {\tt BLS\_G1\_SUB} ( $x$ $y$ -- $x$-$y$) Subtraction on G1.
\item {\tt F93012} --- {\tt BLS\_G1\_NEG} ( $x$ -- $-x$) Negation on G1.
\item {\tt F93013} --- {\tt BLS\_G1\_MUL} ( $x$ $s$ -- $x$*s) Multiplies G1 point $x$ by scalar $s$. Any $s$ is valid, including negative.
\item {\tt F93014} --- {\tt BLS\_G1\_MULTIEXP} ( $x_1$ $s_1$ \ldots $x_n$ $s_n$ $n$ -- $x_1$*$s_1$+\ldots+$x_n$*$s_n$) Calculates $x_1*s_1+\dots+x_n*s_n$ for G1 points $x_i$ and scalars $s_i$. Returns zero point if $n=0$. Any $s_i$ is valid, including negative.
\item {\tt F93015} --- {\tt BLS\_G1\_ZERO} ( -- $zero$) Pushes zero point in G1.
\item {\tt F93016} --- {\tt BLS\_MAP\_TO\_G1} ( $f$ -- $x$) Converts FP element $f$ to a G1 point.
\item {\tt F93017} --- {\tt BLS\_G1\_INGROUP} ( $x$ -- $?$) Checks that slice $x$ represents a valid element of G1.
\item {\tt F93018} --- {\tt BLS\_G1\_ISZERO} ( $x$ -- bool) Checks that G1 point $x$ is equal to zero.
\item {\tt F93020} --- {\tt BLS\_G2\_ADD} ( $x$ $y$ -- $x$+$y$) Addition on G2.
\item {\tt F93021} --- {\tt BLS\_G2\_SUB} ( $x$ $y$ -- $x$-$y$) Subtraction on G2.
\item {\tt F93022} --- {\tt BLS\_G2\_NEG} ( $x$ -- $-x$) Negation on G2.
\item {\tt F93023} --- {\tt BLS\_G2\_MUL} ( $x$ $s$ -- $x$*s) Multiplies G2 point $x$ by scalar $s$. Any $s$ is valid, including negative.
\item {\tt F93024} --- {\tt BLS\_G2\_MULTIEXP} ( $x_1$ $s_1$ \ldots $x_n$ $s_n$ $n$ -- $x_1$*$s_1$+\ldots+$x_n$*$s_n$) Calculates $x_1*s_1+\ldots+x_n*s_n$ for G2 points $x_i$ and scalars $s_i$. Returns zero point if $n=0$. Any $s_i$ is valid, including negative.
\item {\tt F93025} --- {\tt BLS\_G2\_ZERO} ( -- $zero$) Pushes zero point in G2.
\item {\tt F93026} --- {\tt BLS\_MAP\_TO\_G2} ( $f$ -- $x$) Converts FP element $f$ to a G2 point.
\item {\tt F93027} --- {\tt BLS\_G2\_INGROUP} ( $x$ -- $?$) Checks that slice $x$ represents a valid element of G2.
\item {\tt F93028} --- {\tt BLS\_G2\_ISZERO} ( $x$ -- bool) Checks that G2 point $x$ is equal to zero.
\item {\tt F93030} --- {\tt BLS\_PAIRING} ( $x_1$ $y_1$ \ldots $x_n$ $y_n$ $n$ -- $bool$) Given G1 points $x_i$ and G2 points $y_i$, calculates and multiply pairings of $x_i, y_i$. Returns true if the result is the multiplicative identity in FP12, false otherwise. Returns false if $n=0$.
\item {\tt F93031} --- {\tt BLS\_PUSHR} ( -- $r$) Pushes the order of G1 and G2 (approx. $2^{255}$).
\end{itemize}
